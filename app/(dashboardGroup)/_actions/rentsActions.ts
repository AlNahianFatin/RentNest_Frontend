...
export const getMyRentalRecords = async ({ query }: { query?: { [key: string]: string | string[] | undefined } }) => {
    const params = new URLSearchParams()
    ...
    const res = await fetch(`${process.env.BACKEND_API_URL}/api/payments?${params.toString()}`, {
        headers: {
            Cookie: `accessToken=${accessToken}`
        },
        cache: "no-cache"
